...
export const Stats = ({ visualizations, layout, config }: any) => {
  ...
  console.log('satats ==data==', statsData);
  console.log('annotations==', annotations);
  if (chartType === 'horizontal') {
    if (thresholds.length) {
      statsData.map((stat, index) => {
        console.log('stats----', stat, 'index', index);
        thresholds.map((threshold: any, indexT: number) => {
          if (stat.value <= threshold.value) {
            shapes[index].line = {
              color: threshold.color,
              width: 3,
            };
            shapes[index].fillcolor = threshold.color
          }
        });
      });
    } else {
